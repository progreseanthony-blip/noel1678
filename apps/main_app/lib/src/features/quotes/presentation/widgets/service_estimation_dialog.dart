...
class _ServiceEstimationDialogState
        ...
        child: _isLoading
            ...
            : Column(
                children: [
                  ...
                  if (_currentStep == 0) _buildTemplateBanner(),
                  Expanded(
  ...
  Future<void> _showTemplatePicker() async {
    ...
    try {
      templates = await quotesSvc.getEstimationsForTemplate();
    } catch (_) {
      templates = [];
    }
    if (!mounted) return;
    ...
    final result = await showSafeDialog<Map<String, dynamic>>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          title: Text('Load from Existing Projection', style: GoogleFonts.manrope(fontSize: 16, fontWeight: FontWeight.w800)),
          content: SizedBox(
            width: 650,
            height: 500,
            child: Column(
              children: [
                TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'Search by service, quote, or customer...',
                ...
                Expanded(
                  child: templates.isEmpty
                      ? Center(child: Text('No existing projections found', style: GoogleFonts.manrope(color: AppTheme.slate400)))
                      : ListView.separated(
                          itemCount: templates.where((t) {
                            if (query.isEmpty) return true;
                            final svc = (t['quote_services'] as Map<String, dynamic>?) ?? {};
                            final qt = (svc['quotes'] as Map<String, dynamic>?) ?? {};
                            final cust = (qt['customers'] as Map<String, dynamic>?) ?? {};
                            return (svc['name'] ?? '').toString().toLowerCase().contains(query) ||
                                (qt['title'] ?? '').toString().toLowerCase().contains(query) ||
                                (cust['name'] ?? '').toString().toLowerCase().contains(query);
                          }).length,
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final qt = (svc['quotes'] as Map<String, dynamic>?) ?? {};
                            final cust = (qt['customers'] as Map<String, dynamic>?) ?? {};
                            final svcName = svc['name'] ?? 'Unknown Service';
                            final unit = svc['unit_of_measure'] ?? 'CY';
                            final cy = (t['total_cy_loose'] as num?)?.toDouble() ?? 0;
                            final days = (t['total_working_days'] as num?)?.toDouble() ?? 0;
                            final sd = t['start_date'] as String?;
                            final sdFormatted = sd != null ? DateFormat('MMM dd').format(DateTime.parse(sd)) : '—';
                            final quoteTitle = qt['title'] ?? 'Unknown Quote';
                            final customerName = cust['name'] ?? '—';

                            return ListTile(
                              dense: true,
                              title: Text('$svcName — ${_nFmt.format(cy)} $unit', style: GoogleFonts.manrope(fontSize: 13, fontWeight: FontWeight.w700)),
                              subtitle: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text('$quoteTitle · $customerName', style: GoogleFonts.manrope(fontSize: 11, color: AppTheme.slate500)),
                                  Text('${days > 0 ? '${days.toStringAsFixed(0)} working days' : '—'} · $sdFormatted', style: GoogleFonts.manrope(fontSize: 11, color: AppTheme.slate400)),
                                ],
                              ),
                              trailing: TextButton(
                                onPressed: () => Navigator.pop(ctx, t),
                                child: Text('Select', style: GoogleFonts.manrope(fontWeight: FontWeight.w700, color: AppTheme.primaryGreen)),
                              ),
                            );
                          },
                        ),
    ...
    if (result != null && mounted) {
      await _loadFromTemplate(result['id'] as String);
    }
  }

  Future<void> _loadFromTemplate(String estimationId) async {
    setState(() => _isLoading = true);
    try {
      final quotesSvc = ref.read(quotesServiceProvider);
      final full = await quotesSvc.getEstimationFullData(estimationId);
      if (full == null || !mounted) { setState(() => _isLoading = false); return; }

      final svc = (full['quote_services'] as Map<String, dynamic>?) ?? {};
      final qt = (svc['quotes'] as Map<String, dynamic>?) ?? {};
      final svcName = svc['name'] ?? 'Unknown Service';
      final quoteTitle = qt['title'] ?? '—';
      _templateSourceName = '$svcName ($quoteTitle)';

      _topsoilCtrl.text = full['topsoil_volume']?.toString() ?? '0';
      _compactedCtrl.text = full['compacted_volume']?.toString() ?? '0';
      _swellFactorCtrl.text = full['swell_factor']?.toString() ?? '0.15';
      _thicknessCtrl.text = full['thickness_inches']?.toString() ?? '0';
      _gravelThicknessCtrl.text = full['gravel_thickness_inches']?.toString() ?? '0';
      _trenchWidthCtrl.text = full['trench_width_inches']?.toString() ?? '0';
      _trenchDepthCtrl.text = full['trench_depth_inches']?.toString() ?? '0';

      final sd = full['start_date'];
      if (sd is String) {
        _startDate = DateTime.tryParse(sd) ?? DateTime.now();
      ...
      final resList = full['resources'] as List? ?? [];
      _selectedResources = resList.map((r) {
        final qty = (r['quantity'] as num?)?.toDouble() ?? 1.0;
        final trips = (r['trips_per_day'] as num?)?.toDouble() ?? 0.0;
        final cap = (r['capacity_per_trip'] as num?)?.toDouble() ?? 0.0;
        final perf = (r['performance_per_day'] as num?)?.toDouble() ?? 0.0;
        final isPrimary = r['is_primary_mover'] as bool? ?? true;
        final mach = r['machinery'] as Map<String, dynamic>? ?? {};
        return {
          'id': r['id'] ?? _uuid.v4(),
          'is_primary_mover': isPrimary,
          'parent_resource_id': r['parent_resource_id'],
          'machine_id': r['machine_id'],
          'machine_name': mach['description'] ?? 'Unknown',
          'photo_url': mach['photo_url'],
          'machinery_type': mach['machinery_type'] ?? 'hauling',
          'operator_role_id': mach['operator_role_id'],
          'quantity': qty,
          'trips_per_day': trips,
          'capacity_per_trip': cap,
          'performance_per_day': perf,
      ...
      final matList = full['materials'] as List? ?? [];
      _selectedMaterials = matList.map((m) {
        final qty = (m['quantity'] as num?)?.toDouble() ?? 0.0;
        final mats = m['materials'] as Map<String, dynamic>? ?? {};
        return {
          'id': m['id'],
          'material_id': m['material_id'],
          'material_name': mats['description'] ?? 'Unknown',
          'unit': mats['unit'] ?? 'und',
          'yield_factor': (mats['yield_factor'] as num?)?.toDouble() ?? 1.0,
          'quantity': qty,
          'unit_price': (m['unit_price'] as num?)?.toDouble() ?? 0.0,
          'layer_type': m['layer_type'] ?? 'earth',
          'notes': m['notes'],
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Error loading template'), backgroundColor: AppTheme.errorRed),
        );
      }
    } finally {
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Load from existing projection', style: GoogleFonts.manrope(fontSize: 13, fontWeight: FontWeight.w700, color: AppTheme.primaryGreen)),
                      Text('Start with a proven resource configuration', style: GoogleFonts.manrope(fontSize: 11, color: AppTheme.slate500)),
                    ],
                  ),
                ),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Based on: $_templateSourceName', style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.blue)),
                Text('Modify values and save as a new projection', style: GoogleFonts.manrope(fontSize: 10, color: AppTheme.slate500)),
              ],
            ),
          ),
          TextButton(
            onPressed: () { setState(() { _templateSourceName = null; _estimationId = null; }); },
            child: Text('Clear', style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w600, color: AppTheme.errorRed)),
          ),
        ],
      ),
    );
