...
class WeeklyInspectionsProjectsPage extends StatefulWidget {
  const WeeklyInspectionsProjectsPage({super.key});

  @override
  State<WeeklyInspectionsProjectsPage> createState() =>
      _WeeklyInspectionsProjectsPageState();
}

class _WeeklyInspectionsProjectsPageState
    extends State<WeeklyInspectionsProjectsPage> {
      ...
      drawer: isMobile
          ? Drawer(
              ...
              child: Sidebar(
                userName: userName,
                userEmail: userEmail,
                currentPath: '/weekly-inspections',
                onLogout: () async {
            ...
            child: Column(
              children: [
                if (!isMobile)
                  TopHeader(
                    userName: userName,
                    breadcrumbs: const ['Projects', 'Weekly Inspections'],
                  ),
  ...
  Widget _emptyState() {
    return Center(
      child: Column(
        ...
        children: [
          Icon(Icons.satellite_alt, size: 64, color: AppTheme.slate400),
          const SizedBox(height: 16),
          Text(
            'No projects available',
            style: GoogleFonts.manrope(
                fontSize: 16, fontWeight: FontWeight.w700, color: AppTheme.slate600),
          ),
          const SizedBox(height: 8),
          Text(
            'Select a project to manage weekly inspections.',
    ...
    return ListView.separated(
      ...
      itemBuilder: (ctx, i) {
        ...
        return Card(
          ...
          child: InkWell(
            onTap: () =>
                context.push('/projects/${p['id']}/weekly-inspections'),
            borderRadius: BorderRadius.circular(12),
            child: Padding(
