...
class _ServicesTabState extends ConsumerState<ServicesTab> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final data = await ref.read(catalogsServiceProvider).getServices();
      setState(() {
        _services = data;
        _isLoading = false;
        _error = null;
              ...
              child: ListView.separated(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    subtitle: Text('Unit: ${item['unit']}', style: GoogleFonts.manrope(color: AppTheme.slate500, fontSize: 12)),
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
  ...
  Future<void> _deleteItem(String id) async {
    ...
    if (confirm == true) {
      try {
        await ref.read(catalogsServiceProvider).deleteService(id);
        _loadData();
      } catch (e) {
        if (mounted) {
