...
2. *Crear Rama:* Crear una rama `feat/[nombre-funcionalidad]` en minúsculas con guiones.
...
1. *Regreso a Base:* Cambiar a la rama principal (`main`).
2. *Fusión (Merge):* Unificar los cambios de la rama de la funcionalidad usando `git merge --no-ff feat/...` con un mensaje descriptivo.
3. *Limpieza:* Eliminar la rama `feat/` local (`git branch -d feat/...`).
4. *Respaldo Cloud:* Ejecutar `git push origin main`.
5. *Limpieza remota:* Si la rama existe en origin, eliminarla con `git push origin --delete feat/...`.
...
- Siempre verificar `git status` y `git log --oneline -3` antes de cualquier operación destructiva.
